...
function saveQueueCampaigns(list) {
  ...
  try { scheduleOperationalSync({ delay:400, reason:'queue-campaign-update' }); } catch(e){}
}
...
function saveQueueTemplates(list) {
  ...
  uiSyncLog('optimistic-update', { entity:'template', action:'save-queue-cache', count:Array.isArray(list) ? list.length : 0 });
  try { scheduleOperationalSync({ delay:400, reason:'queue-template-update' }); } catch(e){}
}
...
function saveWhatsappQueueV27(list) {
  ...
  uiSyncLog('optimistic-update', { entity:'whatsapp-queue', action:'save-local-cache', count:Array.isArray(list) ? list.length : 0 });
  try { scheduleOperationalSync({ delay:400, reason:'whatsapp-queue-update' }); } catch(e){}
