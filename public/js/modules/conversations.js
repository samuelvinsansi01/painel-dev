/* ════════════════════════════
   CONVERSAS
════════════════════════════ */
let activeConversationLead = null;
...
function getConversationMessages(leadId) {
...
function renderConversationList() {
  ...
  box.innerHTML = items.map(item => {
    ...
    const dateLabel = formatConversationListDate(last?.at || item.lastAt || '');
    return `
...
function renderConversationChat() {
  ...
  const lead = findLeadEverywhere(activeConversationLead) || { nome:'Lead' };
  ...
  box.innerHTML = `
    ...
    <div class="conversation-reply-v38">
      <textarea id="conversationReplyText" placeholder="Responder este lead..."></textarea>
      <button class="btn btn-primary" onclick="sendConversationReply()">Responder</button>
    </div>
  `;
}
...
async function sendConversationReply() {
  ...
  const lead = findLeadEverywhere(activeConversationLead) || {};
  ...
  const queue = getWhatsappQueueV27 ? getWhatsappQueueV27() : [];
  const lastSent = queue.filter(item => item.leadId === activeConversationLead && item.chipInstance).slice(-1)[0];
  ...
  try {
    ...
    const crm = ensureLeadCrm(activeConversationLead, lead);
    ...
    saveLeadCrm(activeConversationLead, crm);
    addLeadHistory(activeConversationLead, `Resposta enviada pela Central de Conversas`, lead);
...
function updateConversationsBadge() {
  ...
  if (!badge) return;
  badge.textContent = getAllConversationLeads().length;
}
